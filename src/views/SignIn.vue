...
<template>
    <div class="signin-container">
        ...
        <form class="flex flex-column gap-3" @submit.prevent="signin">
            ...
            <InputText type="email" v-model="email" placeholder="Your Email" class="input-field" aria-label="Email"/>
            <InputText type="password" v-model="password" placeholder="Password" class="input-field" aria-label="Password"/>
            <div class="loader-wrapper" v-if="authStore.loader">
                <Loader />
            </div>
            <div v-if="!authStore.loader" class="flex flex-column gap-3">
                <Button label="Sign In" type="submit" class="submit-button"/>
                <span class="login-link">
                    Are you not registered yet? <router-link to="/signup" class="custom-link">Sign up</router-link>
                </span>
            </div>
        </form>
    </div>
...
.loader-wrapper {
    ...
    align-items: center;
    height: 50px; /* Set a specific height for loader wrapper */
}
.custom-link {
  color: white; 
